  ...
  const Carousel = ({ isMobile }: { isMobile: boolean }) => (
    <div
      className={`justify-center ${
        isMobile ? 'inline-flex lg:hidden' : 'hidden lg:inline-flex'
      }`}
    >
      <div
      ...
      <div className="absolute mt-[250px] flex justify-between w-full px-5">
        <div
          onClick={prevSlide}
          className="rounded-full p-1 bg-black/20 text-white cursor-pointer"
        >
    ...
    <div className="xs:mx-10 lg:mx-20 xs:mb-5 lg:mb-10 mt-10 flex xs:flex-col lg:flex-row xs:justify-center lg:justify-between gap-10">
      ...
      <div className="flex flex-col xs:max-w-[930px] lg:max-w-[300px] items-center">
        ...
        <div className="flex flex-row my-10">
          {['S', 'M', 'L'].map((size) => (
            <button
              key={size}
              className="mx-5 border-2 px-4 py-1 text-xs font-bold"
            >
              {size}
            </button>
          ))}
        ...
        <div className="flex flex-row my-10">
          {['red-600', 'green-600', 'blue-600', 'yellow-300'].map(
            (color, idx) => (
              <button
                key={idx}
        ...
        <div className="flex flex-col">
          <div className="mt-5 flex justify-center bg-black text-white py-2">
            Price
          </div>
          ...
          <div className="flex flex-row justify-center items-center">
            <input
              ...
              defaultValue={1}
              min={1}
              className="border-2 px-2 w-[70px] text-center"
            />
      ...
      <div className="flex flex-col xs:max-w-[950px] lg:max-w-[400px] xs:items-center lg:items-start">
        ...
        <p className="font-light text-justify">
          ...
          scrambled it to make a type specimen book, it survived five centuries
          and transitioned into electronic typesetting.
        </p>
      </div>
    </div>
  );
